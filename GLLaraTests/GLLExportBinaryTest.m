...
- (void)setUp
{
	...
	XCTAssertNotNil(inMemoryStore, @"Couldn't create store: %@", error);
...
- (void)testBinaryFromASCII
{
	...
	NSError *error = nil;
	NSData *exported = [newItem writeBinaryError:&error];
	XCTAssertNotNil(exported, @"Should have written something");
	XCTAssertNil(error, @"Should not be an error, is %@", error);
	GLLModel *model = [[GLLModel alloc] initBinaryFromData:exported baseURL:baseURL parent:nil error:&error];
	...
	XCTAssertEqualObjects(mesh.displayName, @"Test", @"Incorrect name of mesh");
	...
	XCTAssertEqual([mesh offsetForTangentLayer:0], (NSUInteger) 36, @"Wrong offset");
